...
2. To change 'Fee' and 'Fee per Hour' columns as currency (US Dollars), at Home tab, change the Accounting format into US Dollar like the picture below. 
...
3. Change the format of Start Date type with 'dd-mmmm-yy' to get the show dates like the format: 15-Jan-2024.
...
4. Before standardize the 'Completed' column to only show 'Yes' or 'No', change the words of 'Yes' and 'No' by using Proper formula so that only the first letter is capitalized, making the writing look neater. And then Copy the values into the initial column.
<img width="1920" height="1018" alt="Image" src="https://github.com/user-attachments/assets/fbe15d21-0d3e-40a3-ab25-df53e77bbeda" />

After that, use the ‘Filter’ feature to standardize the ‘Completed’ column so that it only displays ‘Yes’ or ‘No’. We can change it if we want to display one or both.
